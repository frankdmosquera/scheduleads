...
- **The log's feature groups are not yours.** Rerunning refreshes the page's
  project and roadmap views only. Every `view-feature-N` block stays exactly as
  `/feature` and `/implement` left it.
